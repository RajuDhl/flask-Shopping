const tableRef = document.getElementById("repo-log").getElementsByTagName('tbody')[0];
        ...
        const accountingView = {
            ...
            'back_end_product_cancellations':{'type':'numeric', 'display':'Back End Cancellations'},
        ...
        let until = document.getElementById('end-date').value + ' 23:59';
        console.log('until: '+ String(until));

        function set_timePeriod(){
            timePeriod = new Date(document.getElementById('start-date').value);
            until = new Date(document.getElementById('end-date').value + ' 23:59');
            console.log('until: '+ String(until));
            refresh_data();
        }
        ...
        window.onload = function() {
            ...
            let dt = new Date();
            end.value = String(dt.getFullYear()) +'-'+ String("0" + (dt.getMonth() + 1)).slice(-2) +'-'+ String("0" + String((parseInt(dt.getDate())+1)) ).slice(-2);
            set_timePeriod()
        }
            ...
            console.log(typeof(parseFloat(loan_amount)))
                ...
                if (loan_amount!=""){
                    ...
                    loan_amount = parseFloat(loan_amount) 
                        ...
                        loan_amount = parseFloat(loan_amount)+0.1
                    }
                var repo = document.getElementById("repo-log")
                repo.style.display='none';
                var repo_filer =document.getElementById('log_filter')
                log_filter.style.display='block'

                db.collection("repos").where("loan", "==" ,loan_amount)
                .get()
                .then(function (querySnapshot) {
                        querySnapshot.forEach(function (doc) {
